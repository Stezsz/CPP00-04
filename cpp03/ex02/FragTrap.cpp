...
void	FragTrap::highFivesGuys(void) {
	if (_hitPoints && _energyPoints) {
		std::cout << BLUE_TEXT << "FragTrap " << _name
				<< " requests a high five" << std::endl << WHITE_TEXT;
	}
	else if (!_hitPoints)
		std::cout << RED_TEXT << "FragTrap " << _name << " is dead" << std::endl <<  WHITE_TEXT;
	else
		std::cout << RED_TEXT << "FragTrap " << _name << " has no energy points" << std::endl << WHITE_TEXT;
}
